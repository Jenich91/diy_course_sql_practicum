...
select last_name,
       ...
       email,
       phone,
       (select country || ' ' || city || ' ' || street || ' ' || house
        from adress a
...
-- 3 Запрос позволяющий проверить есть ли в группе однофамильцы (не важно студенты или учителя).
-- Вывести все повторяющиеся фамилии и их количество.
...
where role = 'teacher'
group by p.person_id
having sum(time) = (select max(time_sum)
                    from (select sum(time) as time_sum
                          from persons p
                                   join contacts c on p.person_id = c.persons_id
                                   join person_lesson pl on p.person_id = pl.person_id
                                   join lessons l on l.lesson_id = pl.lesson_id
...
select last_name, first_name, group_concat(material_name) as materials
from persons
...
select last_name, first_name, group_concat(distinct material_name) as materials
from persons
         join person_lesson pl on persons.person_id = pl.person_id
         join lessons l on l.lesson_id = pl.lesson_id
         join material_lesson ml on l.lesson_id = ml.lesson_id
...
from persons p
         join person_lesson pl on p.person_id = pl.person_id
         join lessons l on l.lesson_id = pl.lesson_id
         join adress a on a.adress_id = p.adress_id
where role = 'student'
group by p.person_id
having count() = (select max(count)
                  from (select count() as count
...
having sum(time) = (select max(time_sum)
                    from (select sum(time) as time_sum
                          from persons p
                                   join person_lesson pl on p.person_id = pl.person_id
                                   join lessons l on l.lesson_id = pl.lesson_id
                                   join adress a2 on a2.adress_id = p.adress_id
                          where role = 'student'
                          group by p.person_id));
